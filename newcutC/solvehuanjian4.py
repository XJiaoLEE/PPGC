...
# 定义每个不等式右侧的表达式
R1 = 3*(2*x*exp(epsilon) - 2*x - exp(2*epsilon) + 1)/(4*x*(3*exp(2*epsilon) + 4*exp(epsilon) + 1))
R2 = tanh(epsilon/2)/(4*x)
R3 = 3*(2*x*exp(2*epsilon) - 2*x*exp(epsilon) - exp(2*epsilon) + 1)/(4*x*(exp(2*epsilon) + 4*exp(epsilon) + 3))
