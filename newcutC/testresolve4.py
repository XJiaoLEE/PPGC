import numpy as np
from sympy import symbols, exp, Rational, tanh
...
# a2_value = (exp(epsilon/2) - exp(-epsilon/2))/(4*x*(exp(epsilon/2) + exp(-epsilon/2)))
# a1_value = (6*x*exp(epsilon) - 6*x - exp(epsilon) + 1)/(12*x*(exp(epsilon) + 1))

#a2_value = (exp(epsilon/2) - exp(-epsilon/2))/(4*(exp(epsilon/2) + exp(-epsilon/2)))
#a1_value = 5*(exp(epsilon/2) - exp(-epsilon/2))/(12*(exp(epsilon/2) + exp(-epsilon/2)))


a2_value = tanh(epsilon/2)/4
a1_value = -(exp(epsilon)*tanh(epsilon/2) - 6*exp(epsilon) + tanh(epsilon/2) + 6)/(12*exp(epsilon) + 12)
...
x_values = np.linspace(-0.99, 0.99, 20)
...
for x_val in x_values:
    for eps_val in epsilon_values:
        ...
        if any(p < 0 for p in [p1_val, p2_val, p3_val, p4_val]):
            ...
            print(p1_val, p2_val, p3_val, p4_val)
            break
        ...
        if not (privacy_inequality_1_func(x_val, eps_val) and privacy_inequality_2_func(x_val, eps_val) and privacy_inequality_3_func(x_val, eps_val)):
            ...
            print(privacy_inequality_1_func(x_val, eps_val),x_val,eps_val)
            print(privacy_inequality_2_func(x_val, eps_val),x_val,eps_val)
            print(privacy_inequality_3_func(x_val, eps_val),x_val,eps_val)
            all_conditions_met = False
            break
...
output2_squared = (C*1/3)**2

output3_squared = (-(C*1/3))**2
...
variance_simplified = variance.simplify().rewrite(exp)
