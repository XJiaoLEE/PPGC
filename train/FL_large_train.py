...
global_optimizer = optim.Adam(global_model.parameters(), lr=LEARNING_RATE*10)
...
def train_epoch(global_model, global_optimizer, client_datasets, test_loader, mechanism='BASELINE', out_bits=1):
    ...
    for round in range(NUM_ROUNDS):
        ...
        selected_clients = random.sample(range(total_local_clients), total_local_clients // 2.5)  # Randomly select half of the clients
        ...
        for name, param in global_model.named_parameters():
            if param.requires_grad:    
                if name == "module.layer1.0.conv2.weight":
                    ...
                    # print("global name before aggregation",name,param.grad[0][0])  
